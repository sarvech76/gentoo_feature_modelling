...
####### helper functions #######
    def __createFeatures(self):        
        ...
        for useflag in self.useflags:
            if useflag.startswith("+") == True: 
                self.feature_builder.make_binary_feature(useflag[1:], True)
    ...
    def __create_relations_and_groups(self):
        ...
        for groupName, groupMembers in self.ORgrouping.items():
            ...
            # else add them as an OR constraint 
                ...
                log.error(self.packageName + " has more parents " + str(len(parent)) + " in OR group: " +str(groupMembers))
        ...
        for groupName, groupMembers in self.XORgrouping.items():
            ...
            # else add them as an XOR constraint 
                ...
                log.error(self.packageName + " has more parents " + str(len(parent)) + " in XOR group: " +str(groupMembers))
                self.__add_constraint_to_model(self.packageName, groupMembers, "^^")
        ...
        for groupName, groupMembers in self.MAYgrouping.items():
            ...
            # else add them as an MAYBE constraint 
                ...
                log.error(self.packageName + " has more parents " + str(len(parent)) + " in MAYBE group: " + str(groupMembers))
                self.__add_constraint_to_model(self.packageName, groupMembers, "??")
    ...
    def rec_depth(self, depth_dict, g_depth, to_explore):
        if not to_explore:
            #print("g_depth ",g_depth)
            return g_depth
        else:
            node = to_explore.pop()
            #print(node)

            feature = self.feature_model.get_feature(node) 
            if feature.children()!=None:
                for child in feature.children():
                    try:
                        #self.feature_model.get_feature(child.name.str()).children()
                        to_explore.append(child.name.str())
                        depth_dict[child.name.str()] = depth_dict[node]+1
                    except:
                        for ch in child.children():
                            child = ch
                            to_explore.append(child.name.str())
                            depth_dict[child.name.str()] = depth_dict[node]   

                    if depth_dict[child.name.str()] > g_depth:
                        g_depth = depth_dict[child.name.str()]

            #print(g_depth)
            #print(to_explore)
            return self.rec_depth(depth_dict, g_depth, to_explore)
    ...
    def get_depth(self):
        ...
        depth_dict[self.packageName] = 0 
        to_explore.append(self.packageName)                    
        
        self.depth = self.rec_depth(depth_dict, g_depth, to_explore)
        #print("depth ",depth)
        #print(depth_dict)
        return self.depth
    ...
    def __add_constraint_to_model(self, var1, var2, group = ""):
        ...
        if group == "||":
            ...
            # else add them as an XOR constraint 
                ...
                #__add_constraint_to_model(packageName, var2, "^^", builder)
            ...
            if "?" in var2:
                ...
                #cb.implies().openPar().feature(var1_in_var2).implies()
...
####### main functions #######
    def parser(self, reqUse, useflagStr):
    ...
    def createFeatureModel(self, packageName, reqUse, useflagStr):
        ...
        try:
            ...
            log.debug("depth:" + str(self.depth))
